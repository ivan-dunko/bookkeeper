...
# Как пользоваться
Запустить simple_client: 
``` commandline
poetry run python -m bookkeeper.simple_client
```
Добавление категорий/редактирование категорий: возле лейбла Категория (редактируется
категория из комбо-бокса)
Для изменения размера бюджета дважды кликнуть по ячейке таблицы с размером бюджета
и поменять.
Для добавления расхода нажать внизу большую кнопку Добавить и заполнить данные
в диалоговом окне.
